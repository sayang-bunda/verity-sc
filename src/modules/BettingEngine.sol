...
contract BettingEngine {
    function calculateBet(uint256 amount, uint256 poolYes, uint256 poolNo, uint16 feeBps, bool isYes)
        public
        pure
    ...
    function getPrice(uint256 poolYes, uint256 poolNo, bool isYes) public pure returns (uint256 price) {
        return CPMMMath.calcPrice(poolYes, poolNo, isYes);
    }

    function getMinShares(uint256 expectedShares, uint16 slippageBps) public pure returns (uint256 minShares) {
