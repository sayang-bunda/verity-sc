...
contract Verity is ReentrancyGuard, CREAdapter, BettingEngine {
    ...
    function seedLiquidity(
        ...
        _safeTransferFrom(msg.sender, address(this), uint256(amountYes) + uint256(amountNo));
    ...
    function placeBet(
        ...
        m.poolYes                  = uint128(newPoolYes);
        m.poolNo                   = uint128(newPoolNo);
        ...
        if (isYes) {
            ...
            pos.totalBetYes += uint128(amount);
        } else {
            pos.noShares    += uint128(shares);
            pos.totalBetNo  += uint128(amount);
        }
    ...
    function claimPayout(uint256 marketId) external nonReentrant {
        ...
        if (m.status != uint8(DataTypes.MarketStatus.Resolved)) revert Errors.MarketNotActive();
    ...
    function claimRefund(uint256 marketId) external nonReentrant {
        ...
        if (m.status != uint8(DataTypes.MarketStatus.Escalated)) revert Errors.MarketNotActive();
        if (claimed[marketId][msg.sender])                        revert Errors.AlreadyClaimed();
    ...
    function _safeTransferFrom(address from, address to, uint256 amount) internal {
        if (!IERC20(usdc).transferFrom(from, to, amount)) revert Errors.TransferFailed();
    }

    function _safeTransfer(address to, uint256 amount) internal {
        if (!IERC20(usdc).transfer(to, amount)) revert Errors.TransferFailed();
    }
}
